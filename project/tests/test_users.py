...
class TestUserService(BaseTestCase):
    ...
    def test_users(self):
        """Ensure the /ping route does its thing"""
    ...
    def test_add_users(self):
        """Ensure a new users can be added to the database"""
        with self.client:
            ...
            self.assertIn('michael@realpython.com', data['message'])
            self.assertIn('success', data['status'])
    ...
    def test_add_user_invalid_json(self):
        ...
        with self.client:
            response = self.client.post(
                '/users',
                ### why doesn't an empty dict() work here???
                data=json.dumps(dict(herp="derp")),
                content_type='application/json',
            )
            data = json.loads(response.data.decode())
    ...
    def test_add_user_invalid_json_keys(self):
        """Ensure an error is thrown if the json object doesn't have a username key"""
        with self.client:
            response = self.client.post(
                '/users',
    ...
    def test_add_user_duplicate_user(self):
        """ensure error is thrown if the email adready exists"""
        with self.client:
            self.client.post(
                '/users',
                data=json.dumps(dict(
    ...
    def test_single_user(self):
        """Ensure get single user behaves as expected"""
    ...
    def test_all_users(self):
        ...
        user1 = add_user('michael', 'michael@realpython.com', created)
        user2 = add_user('fletcher', 'fletcher@realpython.com')
        with self.client:
